...
2. **ONE PROFESSIONAL LINKEDIN POST** — Mobile-first, scannable, branded. Follow these STRICT rules:
   - HOOK (first 2 lines): A punchy statement or question (drawn from the transcript) that forces readers to click "See more". No preamble, no "In today's post…".
   - BODY: 3-4 short paragraphs, each MAX 2 sentences. Apply Problem → Agitation → Solution flow using insights from the transcript.
   - WHITE SPACE: Use a DOUBLE line break (\\n\\n) between EVERY paragraph for readability on mobile.
   - BULLETS (when listing key takeaways): Use professional symbols 🔹, ✅, or • — one per line, with \\n between bullets and \\n\\n before/after the bullet block.
   - TONE: Professional yet conversational. No corporate jargon, no buzzword soup.
   - LENGTH: 800-1500 characters total.
   - HASHTAGS (MANDATORY): End the post with \\n\\n then a single hashtag line containing EXACTLY 3 hashtags by default — #VidLogicAI plus 2 niche tags relevant to the transcript topic (e.g. #AIAutomation #ContentStrategy). You may add 1-2 more tags only if they add real value. #VidLogicAI must always be present and on the same final line.
   - DISCLOSURE (MANDATORY): After the hashtags line, add \\n then: 🤖 Insights generated by VidLogic AI
   - OUTPUT: Return as a single JSON string with explicit \\n\\n between paragraphs so spacing is preserved exactly when copied.
